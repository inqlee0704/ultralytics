...
from .modules.block3d import C3_3d as C3, SPPF3d, C3k2_3d, C2PSA3d
from .modules.conv3d import Conv3d, DWConv3d
from .modules.head3d import Detect3D
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                args.insert(2, n)  # number of repeats
                n = 1
            if m is C3k2 or m is C3k2_3d:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        elif m is AIFI:
            ...
            c2 = args[1] if args[3] else args[1] * 4
        elif m is nn.BatchNorm2d or m is nn.BatchNorm3d:
            args = [ch[f]]
        elif m is Concat or m is Concat3d:
            c2 = sum(ch[x] for x in f)
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect3D}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, Detect3D}:
                m.legacy = legacy
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m in {CBLinear, TorchVision, Index}:
