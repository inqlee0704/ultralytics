...
# from ultralytics.engine.validator_3d import BaseValidator3D
from ultralytics.utils import LOGGER, ops
from ultralytics.utils.checks import check_requirements
from ultralytics.utils.metrics import ConfusionMatrix, DetMetrics, box_iou_3d
from ultralytics.utils.plotting import output_to_target, plot_images


class DetectionValidator3D(BaseValidator3D):
    """
    A class extending the BaseValidator3D class for validation of 3D detection models.

    Example:
        ```python
        from ultralytics.models.yolo.detect import DetectionValidator3D

        args = dict(model="yolo3d.pt", data="dataset.yaml")
        validator = DetectionValidator3D(args=args)
        validator()
        ```
    """

    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        """Initialize 3D detection model with necessary variables and settings."""
        super().__init__(dataloader, save_dir, pbar, args, _callbacks)
        self.args.task = "detect3d"
        self.args.conf = 0.001
        self.metrics = DetMetrics(save_dir=self.save_dir, on_plot=False)
    ...
    def postprocess(self, preds):
        ...
        return ops.non_max_suppression_3d(
            preds,
            self.args.conf,
            self.args.iou,
            labels=self.lb,
    ...
    def _prepare_pred(self, pred, pbatch):
        """Prepares predictions by scaling boxes to original image size."""
        predn = pred.clone()
        ops.scale_boxes_3d(
            pbatch["imgsz"], predn[:, :6], pbatch["ori_shape"], padding=False
        )  # native-space pred
        return predn
    ...
    def _process_batch(self, detections, gt_bboxes, gt_cls):
        ...
        """
        iou = box_iou_3d(gt_bboxes, detections[:, :6])
        # iou = box_iou(gt_bboxes, detections[:, :4])
        return self.match_predictions(detections[:, 7], gt_cls, iou)
