...
from ultralytics.data.dataset3d import Dataset3D
...
from ultralytics.models.yolo.detect.val_3d import DetectionValidator3D
...
class BaseTrainer3D:
    ...
    def setup_valid_loader(self):
        ...
        valid_dataset = Dataset3D(
            data_dir=self.data['val'],
            split='val',
            transform=self.valid_transform  # Usually no augmentation for validation
        )
    ...
    def setup_valid_transform(self):
        """Setup data validation transform."""
        self.valid_transform = Augment3D(
            size=self.args.imgsz,
            scale=(1.0, 1.0),
            translate=0.0,
            flip_prob=0.0,
        ...
        LOGGER.info("Initialized 3D data validation transform")
